...
interface CalendarProps {
	...
	dayNameFormat?: 'short' | 'single'
}

const Calendar = ({
	data,
	dayNameFormat,
}: { data: MonthDataType[]; dayNameFormat?: 'short' | 'single' }) => {
	...
	const renderMonth = (monthIndex: number) => {
		...
		const daysOfWeek = daysOfWeekMap[dayNameFormat ?? 'short']

		return (
			<section
				key={monthIndex}
				className={cn(
					'reveal relative mx-auto my-10 flex w-full max-h-full max-w-sm flex-col gap-4 overflow-x-clip overflow-y-hidden rounded-3xl bg-zinc-800 p-6 shadow-xl min-[400px]:p-7',
					bgColors(calBgColor),
					{
			...
			>
				<h2
					className={`reveal animate-revealSm text-sm font-bold tracking-wider text-zinc-200 ${takeover && 'invisible '}`}
				>
					{dayjs(currentMonth).format('MMMM')}
				</h2>
				<div className='grid grid-cols-7 gap-2 text-center text-white'>
					{daysOfWeek.map((day) => (
						<p key={day} className='text-xs text-zinc-200/50'>
							{day}
						</p>
					))}
				</div>
